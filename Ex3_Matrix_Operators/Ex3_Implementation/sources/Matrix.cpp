...
namespace zich{
    ...
    Matrix::Matrix(const vector<double> &numbers, int rows, int columns) {
        if (rows <= 0 || columns <= 0){
...
//    }
   ...
   Matrix Matrix ::operator ++(int dummy){
    ...
    Matrix Matrix::operator *(const Matrix &mat) const{
        ...
        Matrix new_mat (this->get_rows(), mat.columns);
    ...
    */
    istream &operator >>(istream &input, Matrix &mat){
        ...
        bool first = true;
        char last_char = '0';
        for (unsigned int i = 0; i < string_mat.length() - 1; ++i) {
            ...
            if (last_char == ']' && (string_mat.at(i) != ',' || i + 1 >= string_mat.length())) {
                cout << "BLA " << string_mat.at(i) << " BLA";
                throw invalid_argument ("The string ',' of the input given is not valid!");
            }
            if (last_char == ',' && string_mat.at(i) != ' ' ) {
                throw invalid_argument ("The string ' ' of the input given is not valid!");
            }
            last_char = string_mat.at(i);
        }
        mat.set_columns(last_col);
        mat.set_rows(count_r);
        mat.set_new_mat(vec);
        return input;
    }
